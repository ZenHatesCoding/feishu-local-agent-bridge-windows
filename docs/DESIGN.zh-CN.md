...
**飞书负责承载人与 Agent 的可见对话和真实通知，Hub 负责机器可判定的对话状态、
上下文权限与回复授权。一个飞书话题就是一段对话，`@` 谁就授予谁一次受控回复；
工作责任只能由显式工作动作改变。**
...
记录一次由 Hub 授权的 `repair` 动作，并向产生该标记的 bridge 返回固定的纠错指令；
bridge 让同一个 Bot 只重做完整标记。畸形文本绝不会创建 dispatch 或真实 `@`；Hub 对
同一活动运行最多允许一次重做，仍不合格则把该运行记录为失败。
...
Pilot 启动的每个 Agent 还会获得一份仅用于本地工具边界的、属于该 Agent 自己的 Hub
能力凭据。普通 bridge 使用常规凭据环境名；如果模型运行时出于安全目的清洗了名称看起来
像凭据的环境变量，协作命令会使用这份显式能力。它只存在于该 Agent 的本地进程树，绝不
进入提示词、日志或账本，也不会让一个 Agent 获得另一个 Bot 的身份。因此文件交付不再
依赖某个模型运行时是否保留环境变量。
