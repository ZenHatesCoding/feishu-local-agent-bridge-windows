...
**多 Agent 协作群：** 多个机器人加入同一个群。一个飞书话题就是一段对话。人和
Agent 可以在话题内彼此回复；只有转交或请求工作时才附带明确的工作语义。下一位
Agent 获得经过授权的结论和持久共享文件，不会获得前一个 Agent 的私有思维链或无关历史。

设计的灵魂是：**共享任务状态，不共享脑内会话**。真实飞书 `@` 是用户可见的
唤醒信号，Hub 的注意力授权（`dispatch`）授权相应的一次运行。Agent 之间回复时两者
必须同时存在，避免意外广播和机器人互相唤醒循环；只有 `handoff`、`ask` 等明确动作
才会改变工作责任。
