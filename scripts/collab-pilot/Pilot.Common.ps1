...
function Test-CollabHubHealth([int]$TimeoutSeconds = 5) {
  if (-not ('System.Net.Http.HttpClientHandler' -as [type])) {
    try { Add-Type -AssemblyName 'System.Net.Http' } catch { }
  }
  $url = "$(Get-CollabHubUrl)/health"
...
function Start-CollabBackground([string]$Name, [string]$ScriptPath, [string[]]$ScriptArguments = @()) {
  ...
  if (Test-CollabPid $table[$Name]) {
    # Keep this function's success stream to its PID return value only.
...
function Stop-CollabRegisteredBridge([object]$Agent) {
  ...
  try {
    ...
    # Prefer the launcher's own filePath (often an absolute path on Windows installs
    # that don't have node.exe on the system PATH). Fall back to whatever node the
    # current PowerShell can resolve.
    $nodePath = if ($Agent.launch.filePath) {
      $expanded = Expand-CollabValue $Agent.launch.filePath
      if (Test-Path -LiteralPath $expanded) { $expanded } else { (Get-Command node.exe -ErrorAction SilentlyContinue).Source }
    } else {
      (Get-Command node.exe -ErrorAction SilentlyContinue).Source
    }
    if (-not $nodePath) {
      Write-Warning "node.exe not found on PATH and the launch filePath is missing; skipping in-bridge kill for $($Agent.id)."
      return
    }
    & $nodePath (Join-Path $script:CollabRepoRoot 'dist\cli.js') kill 1
